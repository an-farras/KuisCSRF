...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
            if (strcmp($_SESSION['token'], $_POST['csrftoken'])) {
                $name = $_POST['name'];
                $email = $_POST['email'];
                $phone = $_POST['phone'];
                $title = $_POST['title'];
        ...
        <div class="card">
            ...
            <h5 class="card-title">Update contact # <?=$contact['id']?></h5>                    
                <form action="update.php?id=<?=$contact['id']?>" method="post">
                    ...
                    <input type="hidden" class="form-control form-control-sm" name="csrftoken" id="csrftoken" value="<?php echo htmlentities($_SESSION['token']); ?>"><br>
